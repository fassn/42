...
/*   Updated: 2019/09/01 01:36:08 by cfargere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			get_wall_texture(int map)
{
	if (map >= 0 && map <= 6)
		return (map - 1);
	if (map == 9)
...
void				draw_wall(t_env *env, int x, int map)
{
	...
	env->wll.tex_i = get_wall_texture(map);
	if (env->sky_mode == 2)
		draw_floor_or_ceiling(env, x, 1, &env->text_wll[TEX_CEIL]);
	else
		draw_sky(env, x, env->wll.drawstart);
	draw_floor_or_ceiling(env, x, 0, &env->text_wll[TEX_FLOOR]);
	draw_wall2(env, x);
}
