...
<body>
    <div>
        <div class="topnav">
            ...
            <a class="active" href="vetements.php">Vetements</a>
        ...
        <div class="tab">
            <button class="tablinks" onclick="openSubcat(event, 'Tous')" id="defaultOpen">Tous les vetements</button>
            <?php

            if ($result = mysqli_query($link, "SELECT DISTINCT `subcat` FROM `products` WHERE cat= 'Vetements'")){
        ...
        <div id="Tous" class="tabcontent"> 
            <div class="element">
                <?php
                if ($data = mysqli_query($link, "SELECT * FROM `products` WHERE cat= 'Vetements'")){
        ...
        foreach ($subcats as $subcat) {?>
            <div id="<?php echo $subcat?>" class="tabcontent">
                <div class="element">
                    <?php
                    if ($result = mysqli_query($link, "SELECT * FROM `products` WHERE cat= 'Vetements' AND subcat IN ('".$subcat."')")){
                        while ($row = mysqli_fetch_assoc($result)){?>
                            <div class="card">
                                <img src='<?php echo $row['img_url'];?>'style="width:100%"/>
                                <h1><?php echo $row['name'];?> </h1>
                                <p class="price"><?php echo $row['price']."$";?></p>
                                <form action="includes/manage_cart.php?<?= "action=add_article&product=" . $row['name'] . "&img=" . $row['img_url'] . "&price=" . $row['price'] . "&quantity=" . '1' ;?>" method="POST">
                                    <p><button type="submit">Ajouter au panier</button></p>
                                </form>
                            </div>
                         <?php }?>
